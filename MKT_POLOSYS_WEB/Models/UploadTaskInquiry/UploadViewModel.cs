using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using MKT_POLOSYS_WEB;

namespace MKT_POLOSYS_WEB.Models.UploadTaskInquiry
{
    public class UploadViewModel
    {
        public string Number { get; set; }
        public string BranchName { get; set; }
        public string Region { get; set; }
        public string TaskID { get; set; }

        public string JenisTask { get; set; }

        public string CustID { get; set; }

        public string CustomerName { get; set; }
        public string DistributedDate { get; set; }
        public string StartedDate { get; set; }
        public string EmpPosition { get; set; }
        public string soa { get; set; }
        public string Referentor1 { get; set; }
        public string RegionalId { get; set; }
        public string Product { get; set; }
        public string CabId { get; set; }
        public string NIK { get; set; }
        public string TempatLahir { get; set; }
        public string TglLahir { get; set; }
        public string AlamatLeg { get; set; }
        public string ProvLeg { get; set; }
        public string KabLeg { get; set; }
        public string KecLeg { get; set; }
        public string KelLeg { get; set; }
        public string RTLeg { get; set; }
        public string RWLeg { get; set; }
        public string AlamatRes { get; set; }
        public string ProvRes { get; set; }
        public string KabRes { get; set; }
        public string KecRes { get; set; }
        public string KelRes { get; set; }
        public string RTRes { get; set; }
        public string RWRes { get; set; }
        public string NoMesin { get; set; }
        public string NoRangka { get; set; }
        public string ItemType { get; set; }
        public string ItemDescription { get; set; }
        public string Mobile1 { get; set; }
        public string Mobile2 { get; set; }
        public string Phone1 { get; set; }
        public string Phone2 { get; set; }
        public string OfficePhone1 { get; set; }
        public string OfficePhone2 { get; set; }
        public string OtrPrice { get; set; }
        public string ItemYear { get; set; }
        public string MonthlyIncome { get; set; }
        public string MonthInstallment { get; set; }
        public string DP { get; set; }
        public string LTV { get; set; }
        public string Tenor { get; set; }
        public string Plafond { get; set; }
        public string Pekerjaan { get; set; }
        public string SisaTenor { get; set; }
        public string TenorId { get; set; }
        public string ReleaseDateBpkb { get; set; }
        public string MaxPastDueDt { get; set; }
        public string Religion { get; set; }
        public string CustomerRating { get; set; }
        public string TanggalJatuhTempo { get; set; }
        public string MaturityDt { get; set; }
        public string StatusCall { get; set; }
        public string AnswerCall { get; set; }
        public string StatusProspek { get; set; }
        public string ReasonNotProspek { get; set; }
        public string Notes { get; set; }
        public string EmpNo { get; set; }


    }
}
